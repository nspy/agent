...
X_Deallocate_Resources(X_ResourcePtr pRes, X_ListPtr pPathList, int freq)
{
    ...
    while (pNode != NULL)
    {
        ...
        X_Resource_Printf("X_Deallocate_Resource: releasing port = %s path-dir = %s dir = %s ",
                Buf,
                X_RES_DIR_TO_STR(pPathData->PathDir),
                X_RES_DIR_TO_STR(pPathData->Dir));
...
X_Update_Resources(X_ResourcePtr pRes, X_ListPtr pPathList, int freq, int enable)
{
    ...
    while (pNode != NULL)
    {
        ...
        X_Resource_Printf("X_Update_Resources: update port = %s path-dir = %s dir = %s enable = %d",
                Buf,
                X_RES_DIR_TO_STR(pPathData->PathDir),
                X_RES_DIR_TO_STR(pPathData->Dir),
                enable);
       ...
       int freq)
{
    ...
    if (pMod->EqType != EqType_EQP8ROADMC40 &&
            pMod->EqType != EqType_EQPEROADMDC &&
            pMod->EqType != EqType_EQPCCM8)
    {
        return 0;
    }
    ...
    if ((pMod->EqType == EqType_EQP8ROADMC40 ||
            pMod->EqType != EqType_EQPEROADMDC) && pPort != pMod->NetPort)
    {
        return 0;
    }

    /** on CCM perform power equalization on the client ports **/
    if (pMod->EqType == EqType_EQPCCM8 && pPort == pMod->NetPort)
    {
        return 0;
    }
